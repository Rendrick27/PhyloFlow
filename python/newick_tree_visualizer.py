...
def remove_single_child_nodes(tree):
    ...
    Args:
        tree (Toytree.Tree): The input tree
                             from which to remove single-child nodes.
...
def tree_generator(infile, outfile):
    """
    Generates a phylogenetic tree from a Newick string
                            and saves it as an SVG file.

    Args:
        infile (str): Path to the input file containing
                    the Newick formatted tree string.
